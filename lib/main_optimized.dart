...
import 'providers/optimized_plots_provider.dart'; // Use optimized provider
...
import 'core/services/optimized_memory_cache.dart'; // Use optimized cache
...
// Custom gradient theme extension - preserved
@immutable
...
Future<void> _initializeOptimizedServices() async {
  ...
  try {
    ...
    await OptimizedMemoryCache.instance.initialize();
    print('✅ Optimized memory cache initialized');
    
    // Initialize unified cache manager
    await UnifiedCacheManager.instance.initialize();
    print('✅ Unified cache manager initialized');
    
    // Initialize optimized plots cache
    await OptimizedPlotsCache.initialize();
    print('✅ Optimized plots cache initialized');
    
    // Initialize optimized tile cache
    await OptimizedTileCache.instance.initialize();
    print('✅ Optimized tile cache initialized');
    
    // Start enhanced startup preloading in background
    _startBackgroundPreloading();
    
    print('🚀 All optimized services initialized successfully');
  } catch (e) {
    print('❌ Error initializing optimized services: $e');
  }
}
...
void _startBackgroundPreloading() {
  ...
  Future.microtask(() async {
    try {
      await EnhancedStartupPreloader.startEnhancedPreloading();
      print('✅ Background preloading completed');
    } catch (e) {
      print('❌ Error in background preloading: $e');
    }
  });
}

class DHAMarketplaceAppOptimized extends StatefulWidget {
  const DHAMarketplaceAppOptimized({super.key});

  @override
  State<DHAMarketplaceAppOptimized> createState() => _DHAMarketplaceAppOptimizedState();
}

class _DHAMarketplaceAppOptimizedState extends State<DHAMarketplaceAppOptimized> {
  ...
  void _logPerformanceMetrics() {
    try {
      ...
      final memoryCacheStats = OptimizedMemoryCache.instance.getStatistics();
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ...
        // Use optimized plots provider
        ChangeNotifierProvider<OptimizedPlotsProvider>(
          create: (context) => OptimizedPlotsProvider(),
        ),
        ...
        builder: (context, languageService, child) {
          return MaterialApp(
            ...
            theme: ThemeData(
              ...
              // Custom gradient theme - preserved
