...
import '../core/services/tileserver_boundary_service.dart';
...
/// Enhanced Projects Screen that uses tileserver instead of GeoJSON
class ProjectsScreenTileserver extends StatefulWidget {
  const ProjectsScreenTileserver({super.key});

  @override
  State<ProjectsScreenTileserver> createState() => _ProjectsScreenTileserverState();
}

class _ProjectsScreenTileserverState extends State<ProjectsScreenTileserver>
  ...
  // Tileserver boundary data (NO GeoJSON)
  ...
  bool _showConnectionTest = false;
  ...
  Future<void> _testTileserverConnection() async {
    ...
    try {
      print('🔄 Testing tileserver connection...');
      
      // Test connection
      final status = await TileserverBoundaryService.testConnection();
      setState(() {
        _connectionStatus = status;
        _tileserverConnected = status['server_running'] == true;
        _showConnectionTest = true;
      });

      if (_tileserverConnected) {
        print('✅ Tileserver connected successfully');
        
        // Get available phases from tileserver (NO GeoJSON loading)
  ...
  // NO GeoJSON loading - boundaries come from tileserver tiles
        ...
        children: [
          ...
          FlutterMap(
            ...
            children: [
              ...
              // Tileserver boundary layers - REPLACE GeoJSON with MBTiles
              if (_tileserverConnected && _showBoundaries) ...[
                // Load all available phases as tile layers
          ...
          // Loading indicator
          if (_isLoading || _isLoadingBoundaries)
            Container(
  ...
  /// Get tileserver boundary layers - REPLACES GeoJSON boundaries
  List<TileLayer> _getTileserverBoundaryLayers() {
    ...
    // Create tile layers for each available phase
  ...
  /// Get phase label markers for tileserver boundaries
    ...
    for (final phase in _availablePhases) {
      ...
      final marker = Marker(
        ...
        child: EnhancedPhaseLabel(
          phaseName: phase,
          color: DHATileserverService.getPhaseColor(phase),
          icon: DHATileserverService.getPhaseIcon(phase),
          position: center,
          zoom: _zoom,
        ),
      );
